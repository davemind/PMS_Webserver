...
for camera in cameras:
	if camera['state'] == 1:
		command = 'gst-launch-1.0.exe rtspsrc location="' + camera['camera_url'] + '" ! rtph264depay ! avdec_h264 ! videoconvert ! clockoverlay ! x264enc ! mpegtsmux ! hlssink location='\
		...
		if camera['state'] == 1:
			if camera['id'] in proc_dict:
				if proc_dict[camera['id']] is None:
					command = 'gst-launch-1.0.exe rtspsrc location="' + camera[
...
def playOneCamera():
	...
	if camera_id in proc_dict :
		if proc_dict[camera_id] is None:
			command = 'gst-launch-1.0.exe rtspsrc location="' + camera_url + '" ! rtph264depay ! avdec_h264 ! videoconvert ! clockoverlay ! x264enc ! mpegtsmux ! hlssink location=' \
