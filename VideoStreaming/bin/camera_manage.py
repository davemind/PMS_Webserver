		...
		if camera['state'] == 1:
			if camera['id'] in proc_dict:
				if proc_dict[camera['id']] is None:
					command = 'gst-launch-1.0.exe rtspsrc protocols=tcp location="' + camera['camera_url'] + '" ! rtph264depay ! h264parse ! mpegtsmux ! hlssink location=' + camera['camera_name'] + '.%05d.ts playlist-location=' + camera['camera_name'] + '.m3u8'
					proc = subprocess.Popen(command)
					proc_dict[camera['id']] = proc
			else:
				command = 'gst-launch-1.0.exe rtspsrc protocols=tcp location="' + camera['camera_url'] + '" ! rtph264depay ! h264parse ! mpegtsmux ! hlssink location=' + camera['camera_name'] + '.%05d.ts playlist-location=' + camera['camera_name'] + '.m3u8'
...
if __name__ == "__main__":
	schedule.every(10).seconds.do(playCamera)
	while True:
